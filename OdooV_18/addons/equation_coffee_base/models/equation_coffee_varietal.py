...
class EquationCoffeeVarietal(models.Model):
    ...
    _name = 'equation.coffee_varietal'
    _description = 'Equation Coffee Varietal'
        ...
        for rec in self:
            if self.search_count([('name', '=ilike', rec.name)]) > 1:
                raise ValidationError(
                    _(f"A varietal with this name {rec.name} already exists. Please contact your administrator."))
