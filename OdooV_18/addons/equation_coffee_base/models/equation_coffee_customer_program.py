...
class EquationCoffeeCustomerProgram(models.Model):

    _name = 'equation.coffee_customer_program'
    _description = 'Equation Coffee Customer Program'
        ...
        for rec in self:
            if self.search_count([('name', '=ilike', rec.name)]) > 1:
                raise ValidationError(
                    _(f"A customer coffee program with this name {rec.name} already exists. Please contact your administrator."))
