...
class EquationCoffeeProfile(models.Model):

    _name = 'equation.coffee_profile'
    _description = 'Equation Coffee Profile'
        ...
        for rec in self:
            if self.search_count([('name', '=ilike', rec.name)]) > 1:
                raise ValidationError(
                    _(f"A coffee profile with this name {rec.name} already exists. Please contact your administrator."))
