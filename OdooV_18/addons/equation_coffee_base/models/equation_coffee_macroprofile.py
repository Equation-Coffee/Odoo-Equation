...
class EquationCoffeeMacroprofile(models.Model):

    _name = 'equation.coffee_macroprofile'
    _description = 'Equation Coffee Macroprofile'
        ...
        for rec in self:
            if self.search_count([('name', '=ilike', rec.name)]) > 1:
                raise ValidationError(
                    _(f"A macroprofile with this name {rec.name} already exists. Please contact your administrator."))
