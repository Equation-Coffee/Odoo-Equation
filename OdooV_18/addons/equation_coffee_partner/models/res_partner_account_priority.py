...
class PartnerAccountPriority(models.Model):

    _name = 'res.partner_account_priority'
    _description = 'Partner Account Priority'
        ...
        for rec in self:
            if self.search_count([('name', '=ilike', rec.name)]) > 1:
                raise ValidationError(
                    _(f"A Registry with this name {rec.name} already exists. Please contact your administrator."))
